...
- **Currency symbol shown wherever there's room** (`withSymbol()`), including every shared
  receipt: a real glyph for USD/EUR/GBP/JPY/INR/PKR/CAD/AUD ($ € £ ¥ ₹ ₨ — CAD/AUD both use `$`,
  same as USD), the plain code as a stand-in for the rest (`AED 100.00`, no single
  ...
  Calculator quick-jump buttons, the Current Sale list's own Subtotal/Tax/Total columns). The PDF
  uses a separate `pdfWithSymbol()` that falls back to the code for ₹/₨ specifically, since
  jsPDF's built-in font can't render them (not in WinAnsi encoding — would come out as
  missing-glyph boxes); the on-screen version still shows the real glyph, full Unicode support
  there.
...
showing the currency symbol/code wherever there's room, in every screen and every shared receipt
(`withSymbol()`/`pdfWithSymbol()` — see §7 for exactly where, §10 for why some spots are
deliberately left out). Verified via CDP across USD/INR/AED: real glyphs on-screen where supported,
the tight spots (LCD digits, Calculator quick-jump buttons, Current Sale list's own Subtotal/Tax/
Total columns) correctly staying symbol-free, and a generated INR PDF rendered to confirm the
WinAnsi-safe "INR" fallback (no missing-glyph box) while the on-screen ₹ renders as a real glyph.
Before that: letting **Share…** hand the PDF to the OS share sheet as an actual file (`sharePdf()`,
...
- **A currency's `symbol` is a real glyph only where one is genuinely standard and Latin-script
  ...
  The PDF goes one step further with its own `pdfWithSymbol()`, falling back to the code for ₹/₨
  too — jsPDF's built-in fonts are WinAnsi-encoded and can't render either glyph (would come out as
  missing-glyph boxes) — while the on-screen version keeps the real glyph, since a web font has
...
- The last app-code commit was **"Show the currency symbol wherever there's room, and in every
  share"** (hash `98e2140` as of this writing); this `CLAUDE.md` update is committed immediately
