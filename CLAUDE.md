...
- **Share Sale panel**: Email / WhatsApp / Copy / Share… / **Download PDF**. Email/WhatsApp/Copy
  ...
  line item, **Sl.no / Item name / Qty / Subtotal / Discount / Tax / Total** columns: Sl.no
  center-aligned; blank item name shows **NA**; discount/tax shown as their rate, "-" when zero;
  Qty includes the unit, e.g. "5 kg"; **Subtotal** shows the line's subtotal with a "(qty x unit
  price)" breakdown alongside it, in place of a separate Unit Price column — followed by a
  **Totals** row (Subtotal/Tax/Total each under its own column) and a bold **Grand Total** row, the
  amount in words, and the "Generated by bconnTech Calculator" footer. Filename either way:
...
narrowing the PDF's currency display after feedback that showing it on every item row was noisy —
...
- The last app-code commit was **"Scope PDF currency display to column headings and Grand Total
  only"** (hash `bac89a4` as of this writing); this `CLAUDE.md` update is committed immediately
