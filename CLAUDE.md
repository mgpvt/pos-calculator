...
- **Download PDF** (`generatePdf()`) — builds the receipt directly with **jsPDF** +
  **jspdf-autotable** (loaded from cdnjs — see §2) and calls `doc.save(...)`, which downloads the
  file straight from the browser. **Does not use `window.print()` or a print dialog at all** —
  that's deliberate, see §10's "why jsPDF, not print" entry. No `<a download>` link, no hidden
  print-only DOM, no `@media print` CSS (removed).
...
- **Share Sale panel**: Email / WhatsApp / Copy / Share… / **Download PDF**. Email/WhatsApp/Copy/
  Share… all use one formatted plain-text receipt (`receiptText()`), headed with the shop name (or
  the "Generated by bconnTech" fallback — see below). **Download PDF** (`generatePdf()`) builds a
  separate jsPDF document (see §5) with one row per line item — **Sl.no / Item name / Qty /
  Subtotal / Discount / Tax / Total** columns: Sl.no center-aligned; blank item name shows **NA**;
  discount/tax shown as their rate, "-" when zero; Qty includes the unit, e.g. "5 kg"; **Subtotal**
  shows the line's subtotal with a "(qty x unit price)" breakdown alongside it, in place of a
  separate Unit Price column — followed by a **Totals** row (Subtotal/Tax/Total each under its own
  column) and a bold **Grand Total** row, the amount in words, and the "Generated by bconnTech
  Calculator" footer. `doc.save(...)` downloads it as `<Shop Name>_receipt.pdf`.
...
replacing the `window.print()`-based PDF with a client-side jsPDF/jspdf-autotable build
(`generatePdf()`), because the browser's own print header/footer (showing the source URL) — raised
twice by the user — has no code-level fix while staying on `window.print()`; the user explicitly
chose to add the dependency rather than live with the limitation (see §10). Verified by
intercepting `URL.createObjectURL` to capture the actual PDF bytes (headless Chrome cancels the
real download — see §9) and rendering them in Chrome's own PDF viewer: no header/footer/URL
anywhere, and the Sl.no/Item name/Qty/Subtotal-breakdown/Discount/Tax/Total layout with the aligned
Totals footer all render correctly. Implemented, verified, committed, and deployed.
...
- **Inside the Claude "Artifact" preview sandbox, `doc.save()`-style script-driven downloads are
  ...
  back to). `navigator.share()` and clipboard access may also be blocked/degraded there. All of it
  works fully on the deployed GitHub Pages URL, the local file, and real mobile/desktop browsers —
  that's the environment these features were designed and tested for. Worth telling the user this
  explicitly if they ever test Download PDF from the Artifact rather than the live site.
...
- **Headless Chrome can be driven interactively via CDP, not just screenshotted at load.** This
  ...
  `generatePdf`, `render`) are invisible to a bare `Runtime.evaluate` reference and throw
  `not defined` — drive them through the real DOM (`document.getElementById(...).click()`), never
  by name. This was used this session to actually exercise the Item Name autofill/storage logic
...
- **Verifying `generatePdf()`'s actual output**: headless Chrome's automation-download plumbing
  cancels jsPDF's real `doc.save()` blob download every time (`Browser.downloadWillBegin` fires,
...
- The last app-code commit was **"Generate the PDF client-side with jsPDF; no more print
  dialog"** (hash `a6d4203` as of this writing); this `CLAUDE.md` update is committed immediately
...
3. Verify changes visually before shipping — there's no test suite, so use headless Chrome
   ...
   `URL.createObjectURL` interception technique in §9 if the change touches `generatePdf()`/the
   PDF. For animation/motion tweaks, prefer a static isolated-HTML swatch over trying to screenshot
   the transition itself (see §9).
...
`--print-to-pdf` concern — it's generated by `generatePdf()` (jsPDF), so verify it with the
`URL.createObjectURL`-interception technique in §9 instead. See §9 also for the two other
