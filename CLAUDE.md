...
- **Overall Discount**: a **flat money amount** typed in by hand (not a percentage), shown just
  above the Summary cards with the currency symbol/code as a prefix label (`data-role="overallDiscCur"`,
  refreshed by `render()` from `curSymbol()`) — a one-off "knock $30 off this whole sale" on top of
  whatever per-item discounts were already applied, not a replacement for them. `saleSums()` clamps
  it to the sale's own pre-discount total so it can never push the grand total negative; the
  `overallDiscAmt` span to the right only appears when that clamp actually bit (showing the effective
  amount), otherwise it would just repeat the number in the input. Resets on **AC**; doesn't persist
  across reloads (it's per-sale, like a per-item discount, not a device setting like the tax rate).
  `saleSums()` returns the *final*, post-overall-discount amount as `grand` (so the Summary card,
  `amountInWords`, `receiptText()`, the PDF, and the email subject all pick it up automatically) plus
  `preDiscountGrand`/`overallDiscountAmt` for whichever of those needs to show the breakdown
  explicitly.
...
- **Overall Discount is a flat money amount off the already-summed grand total, applied once, not a
  ...
  X% off this one item") — so it lives in its own control above the Summary cards rather than being
  folded into the per-item fields or the Qty/Price/Discount/Tax buffer system. It was a
  0-100 percentage field originally; changed to a hand-entered value on user feedback that the shop
  wanted to just type the dollar figure. The input keeps whatever's typed; `saleSums()` clamps the
  *effective* amount to the sale's own pre-discount total (`Math.min(amount, base.grand)`) so it can
  never push the final total negative, and the small span beside the field surfaces that clamped
  amount only when the clamp actually changed the number.
- **`saleSums()` returns the post-overall-discount amount as `grand`, not the raw item sum**,
  ...
  text receipt's extra line) reach for the additional `preDiscountGrand`/`overallDiscountAmt`
  fields. `render()`'s own separate inline copy of that same summing reduce
