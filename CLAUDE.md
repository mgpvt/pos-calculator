...
- **No frameworks, no npm, no package manager** — but `pos-calculator.html` is **no longer
  zero-dependency**: it loads **jsPDF** and **jspdf-autotable** from cdnjs (two `<script>` tags,
  pinned versions 2.5.1 / 3.8.2) to build the PDF receipt directly, entirely client-side (see §5,
  §10). This is the one external dependency in either file; `calculator.html` remains
  dependency-free.
...
- **Share…** (`sharePdf()`) — builds the PDF (see below) and hands it to `navigator.share()` as a
  `File` (`{files:[file], title, text}`) so it can go straight to WhatsApp/Email/any installed app
  — this is the "share the PDF on mobile" path. Falls back to a plain `doc.save(...)` download if
  `navigator.canShare({files:[...]})` isn't supported on that browser/device. The button is only
  shown (`hidden` removed) if `navigator.share` exists at all.
- **Download PDF** (`downloadPdf()`) — the plain "on the web" path: builds the receipt with
  **jsPDF** + **jspdf-autotable** (loaded from cdnjs — see §2) via the shared `buildReceiptPdf()`
  and calls `doc.save(...)` directly, no share sheet. Neither this nor Share… uses `window.print()`
  or a print dialog at all — that's deliberate, see §10's "why jsPDF, not print" entry. No
  `<a download>` link, no hidden print-only DOM, no `@media print` CSS (removed).
...
- **Overall Discount**: a percentage field (0-100, clamped) shown just above the Summary cards,
  applied to the whole sale's already-summed grand total — a one-off "10% off this sale" on top of
  whatever per-item discounts were already applied, not a replacement for them. Resets on **AC**;
  doesn't persist across reloads (it's per-sale, like a per-item discount, not a device setting
  like the tax rate). `saleSums()` returns the *final*, post-overall-discount amount as `grand` (so
  the Summary card, `amountInWords`, `receiptText()`, the PDF, and the email subject all pick it up
  automatically) plus `preDiscountGrand`/`overallDiscountPct`/`overallDiscountAmt` for whichever of
  those needs to show the breakdown explicitly.
- **Summary**: Subtotal / Total Tax / Grand Total cards (Grand Total already net of the Overall
  Discount above), plus the grand total spelled out in cheque form ("Twelve thousand seven hundred
  sixteen and 78/100") below them.
- **Share Sale panel**: Email / WhatsApp / Copy / Share… / **Download PDF**. Email/WhatsApp/Copy
  use one formatted plain-text receipt (`receiptText()`), headed with the shop name (or the
  "Generated by bconnTech" fallback — see below). **Share…** and **Download PDF** both build the
  ...
  *pre*-overall-discount sum), an **Overall Discount (N%)** row when one's set, and a bold
  **Grand Total** row (the final, post-discount amount) with a thick rule above it, then the amount
  in words and the "Generated by bconnTech Calculator" footer. Filename either way:
  `<Shop Name>_receipt.pdf`. Currency in the PDF is scoped deliberately narrow (see §10): the
  Subtotal/Total column *headings* carry it ("Subtotal (INR)"), and only the Grand Total line
  shows it directly on the value ("INR 92.70") — every item row and the Totals row stay plain
  numbers.
...
- **Currency symbol shown wherever there's room, on-screen and in the plain-text receipt**
  ...
  widely-recognized Latin-compatible glyph exists for the Gulf currencies) — see §10 for exactly
  which surfaces get it and which are deliberately left alone (the LCD digits, the narrow
  Calculator quick-jump buttons, the Current Sale list's own Subtotal/Tax/Total columns). **The
  PDF is scoped narrower still** (see above and §10) — plain code only, never a glyph, and only on
  the money columns' headings and the Grand Total value, not on every row.
...
centering the PDF's Qty/Discount/Tax columns to their headers (Subtotal/Total stay right-aligned)
and adding the Overall Discount feature (a percentage off the whole sale's grand total, on top of
any per-item discounts — see §7/§10). Verified via CDP: a 10% overall discount correctly dropping a
$100 subtotal to a $90 grand total on-screen, in the words line, and in a generated PDF (Totals row
still shows the pre-discount amount, a new "Overall Discount (10%)" row, Grand Total shows the
final amount); an out-of-range 150% clamping to 100%; and AC resetting it to 0. Implemented,
verified, committed, and deployed. Before that: narrowing the PDF's currency display after feedback
...
`pdfWithSymbol()` (its WinAnsi-glyph-fallback no longer has anything to fall back for) was removed.
On-screen/text-receipt currency display (`withSymbol()`) is unchanged from before. Verified by
generating and rendering an INR PDF to confirm the new layout. Before that: showing the currency
symbol/code wherever there's room, in every screen and every shared receipt — see §7 for exactly
where, §10 for why some spots are deliberately left out. Before that: letting **Share…** hand the
PDF to the OS share sheet as an actual file (`sharePdf()`,
so it can go straight to WhatsApp/Email/any app on mobile — "Download PDF" stays a plain save via
`downloadPdf()`, both built on a shared `buildReceiptPdf()`), and fixing the Unit field so its
datalist shows every option again instead of only ones matching whatever's already typed. Before
that: replacing the `window.print()`-based PDF with a client-side jsPDF/jspdf-autotable build,
because the browser's own print header/footer (showing the source URL) had no code-level fix while
staying on `window.print()` — see §10.
...
- **PowerShell + UTF‑8 gotcha (already hit once, fixed):** `Get-Content -Raw` in Windows
  ...
  **Bash** tool (`perl`/`sed`, which are byte-safe) — never via `Get-Content` → PowerShell string →
  `WriteAllText`.
- **Inside the Claude "Artifact" preview sandbox, `doc.save()`-style script-driven downloads are
  inert, not just degraded** — the Artifact viewer sandbox blocks any download a page starts
  itself. That means **Download PDF will not work at all in the Artifact preview** (it did
  "work" there before, in the sense that it opened a print dialog; now there's no dialog to fall
  back to), and **Share… falls back to that same inert `doc.save()`** whenever `navigator.share`/
  `canShare` aren't usable in that sandbox either — not verified either way inside an actual
  Artifact preview, so don't assume it works there without checking first. Clipboard access may
  also be blocked/degraded there. All of it works fully on the deployed GitHub Pages URL, the local
  file, and real mobile/desktop browsers — that's the environment these features were designed and
  tested for. Worth telling the user this explicitly if they ever test either PDF button from the
  Artifact rather than the live site.
...
- **Headless Chrome can be driven interactively via CDP, not just screenshotted at load.** This
  ...
  `Page.captureScreenshot` to save a PNG). Functions declared inside the app's own closure (e.g.
  `buildReceiptPdf`, `downloadPdf`, `sharePdf`, `render`) are invisible to a bare `Runtime.evaluate`
  reference and throw `not defined` — drive them through the real DOM
  (`document.getElementById(...).click()`), never by name. This was used this session to actually exercise the Item Name autofill/storage logic
  end-to-end (not just eyeball the layout) and is worth reaching for again whenever a change is
  behavioral, not just visual. **After editing the file, there's no need to relaunch Chrome** —
  `GET http://localhost:9333/json/list` finds the still-open tab, then `Page.reload` (or
...
- **Verifying `downloadPdf()`/`buildReceiptPdf()`'s actual output**: headless Chrome's automation-
  download plumbing cancels jsPDF's real `doc.save()` blob download every time (`Browser.downloadWillBegin` fires,
  `Browser.downloadProgress` even reports the full byte count received, then immediately reports
  `state: "canceled"`) — this is a headless-only quirk, not something a real user hits. To verify
  the PDF's actual bytes, intercept `URL.createObjectURL` before clicking the button (jsPDF's save
  path always funnels the Blob through it, however that particular jsPDF build implements `.save`):
  ```js
  window.__pdfDataUri = null;
  var _orig = URL.createObjectURL;
  URL.createObjectURL = function (blob) {
    blob.arrayBuffer().then(function (buf) {
      var bytes = new Uint8Array(buf), bin = "";
      for (var i = 0; i < bytes.length; i++) bin += String.fromCharCode(bytes[i]);
      window.__pdfDataUri = "data:application/pdf;base64," + btoa(bin);
    });
    return _orig.call(URL, blob);
  };
  ```
  then click the button, poll `window.__pdfDataUri` (the `arrayBuffer()` resolution is async — give
  it a beat), base64-decode it to a `.pdf` file, and open that file in a fresh tab (Chrome's
  built-in PDF viewer) to screenshot the actual rendered page and confirm there's no header/footer
  and the layout is right. To verify `sharePdf()` specifically, `navigator.share`/`canShare` don't
  exist in this headless environment at all, so stub both before clicking Share…
  (`navigator.share = function(data){ window.__shareCalls.push(data); return Promise.resolve(); }`,
  `navigator.canShare = function(){ return true; }`) and inspect what was pushed (a `File` in
  `data.files[0]`, its `.name`/`.type`) rather than trying to exercise the real API.
...
- **Cheque-style amount-in-words ("...and NN/100")** was chosen over a currency-specific phrasing
  ...
  trailing "(CODE)", e.g. "...and 50/100 (USD)" — but still deliberately the *code*, not the glyph:
  "(₹)" or "(AED)" grafted onto the end of a words phrase reads worse than "(INR)"/"(AED)" would,
  and a plain code needs no font-safety consideration at all (see the PDF note below), unlike a
  glyph might.
- **Currency symbol shown only where it doesn't risk breaking a tight layout, not blanket-applied
  to every `money()` call.** The 8-digit-cap tiered-shrink LCD (the point above) already exists
  because that display is fighting for room; gluing a symbol onto the digits there — or onto the
  narrow Calculator quick-jump buttons, or the Current Sale list's already-cramped-on-mobile
  Subtotal/Tax/Total columns — would undo that work. Putting the symbol on the LCD's *label*
  ("Total ($)") instead of the value sidesteps this while still surfacing the currency. Every
  roomy spot (Sale Details rows, Summary cards, the Current Sale list's main item line, every
  shared receipt) gets it directly on the value via a shared `withSymbol()` helper, since that's
  where "wherever there's room" was explicitly asked for.
- **A currency's `symbol` is a real glyph only where one is genuinely standard and Latin-script
  compatible ($ € £ ¥ ₹ ₨); everything else (AED/SAR/QAR/KWD/BHD/OMR/EGP) just uses its own code as
  the "symbol"** ("AED 100.00"), rather than reaching for an Arabic-script mark that would read
  oddly mixed into an otherwise-Latin, LTR-styled UI and isn't reliably supported everywhere either.
  This only matters on-screen/in the text receipt, though — see the next point for the PDF, which
  sidesteps the whole glyph-safety question by never using one.
- **The PDF never uses a symbol glyph at all — plain currency code only, and only in two places:
  the money columns' headings and the Grand Total line.** Originally the PDF also prefixed every
  item row and the Totals row with a currency mark (matching on-screen behavior, with a
  WinAnsi-safe-glyph fallback for jsPDF's built-in font, since ₹/₨ aren't in that encoding and would
  render as missing-glyph boxes) — feedback called that noisy repetition down the page. Now
  `buildReceiptPdf()` puts "(CODE)" on the **Subtotal**/**Total** column headings and prefixes only
...
- **Overall Discount is a percentage of the already-summed grand total, applied once, not a
  per-item field.** It answers a different question than the existing per-line Discount % ("knock
  X% off this whole sale," e.g. a loyalty/coupon discount, vs. "knock X% off this one item") — so
  it lives in its own control above the Summary cards rather than being folded into the per-item
  fields or the Qty/Price/Discount/Tax buffer system. Clamped to 0-100 so it can never push the
  final total negative.
- **`saleSums()` returns the post-overall-discount amount as `grand`, not the raw item sum**,
  specifically so every *existing* caller (the Summary card, `amountInWords`, `receiptText()`, the
  PDF, the email subject) picks up the discount for free without being individually touched — only
  the few spots that need to show the discount's own breakdown (the PDF's extra footer row, the
  text receipt's extra line) reach for the additional `preDiscountGrand`/`overallDiscountPct`/
  `overallDiscountAmt` fields. `render()`'s own separate inline copy of that same summing reduce
  was replaced with a call to `saleSums()` at the same time, so there's only one place that combines
  per-item sums with the overall discount rather than two that could drift apart.
- **PDF's "Totals" row keeps the pre-discount sum, not the final total** — it's meant to read as
  "this is what the line items add up to," with the Overall Discount (when set) and Grand Total
  rows below it showing what happens next, the same way the per-item Subtotal → Discount Amount →
  Tax Amount → Total flow already reads top-to-bottom in Sale Details.
...
- **Switched the PDF from `window.print()`/`@media print` to client-side generation with jsPDF,
  breaking the project's zero-dependency rule on purpose.** The user asked twice for the printed
  page's browser-added header/footer (showing this page's URL) to be removed; that header/footer
  is the print dialog's own "Headers and footers" setting, which no web-standard API lets a page
  disable — there was genuinely no CSS/JS fix available while staying on `window.print()`. Given
  the choice between "explain the limitation" and "actually make the URL impossible to see," the
  user chose the latter explicitly (asked, didn't assume) even knowing it meant a real
  architectural change: this is now the one external dependency in the project, and the Download
  PDF button needs internet the first time a browser loads the page. Old `fillReceiptHTML()` /
  `#receipt` / `@media print` CSS were deleted rather than left as dead code once nothing called
  `window.print()` anymore.
- **PDF's Sl.no/Qty/Discount/Tax columns are center-aligned, Subtotal/Total stay right-aligned** —
  set via `columnStyles` in the `doc.autoTable(...)` call (`buildReceiptPdf()`), which applies to
  header and body cells alike for that column index. Sl.no was the first to get this (it was
  right-aligned like the money columns, which put its numbers out of step with the header); Qty/
  Discount/Tax followed on later feedback, since they're not money amounts and read better centered
  under their own headers rather than right-aligned as if they were.
...
3. Verify changes visually before shipping — there's no test suite, so use headless Chrome
   screenshots (see §13) for both a desktop-width and a mobile-width (≤760px) render, and the
   `URL.createObjectURL` interception technique in §9 if the change touches
   `buildReceiptPdf()`/`downloadPdf()`/`sharePdf()`. For animation/motion tweaks, prefer a static
   isolated-HTML swatch over trying to screenshot the transition itself (see §9).
...
```
Repeat with `--window-size` ≤760 width for the mobile layout. The receipt itself is no longer a
`--print-to-pdf` concern — it's generated by `buildReceiptPdf()` (jsPDF), downloaded via
`downloadPdf()` or shared via `sharePdf()`, so verify it with the `URL.createObjectURL`-interception
technique in §9 instead. See §9 also for the two other
headless-Chrome quirks to watch for (viewport cropping, and unreliable mid-transition capture with
`--virtual-time-budget`).
