...
letting **Share…** hand the PDF to the OS share sheet as an actual file (`sharePdf()`, so it can go
straight to WhatsApp/Email/any app on mobile — "Download PDF" stays a plain save via
`downloadPdf()`, both built on a shared `buildReceiptPdf()`), and fixing the Unit field so its
datalist shows every option again instead of only ones matching whatever's already typed (see §7).
Verified via CDP: focus/blur clearing and restoring the Unit field correctly, `downloadPdf()` still
producing a valid PDF, and `sharePdf()` correctly calling a stubbed `navigator.share()` with an
attached `application/pdf` File (`navigator.share`/`canShare` aren't available in this headless
environment, so they were stubbed to exercise the call). Implemented, verified, committed, and
deployed. Before that: replacing the `window.print()`-based PDF with a client-side
jsPDF/jspdf-autotable build, because the browser's own print header/footer (showing the source URL)
had no code-level fix while staying on `window.print()` — see §10.
...
- The last app-code commit was **"Share the PDF via the OS share sheet on mobile; fix unit
  datalist"** (hash `44122a2` as of this writing); this `CLAUDE.md` update is committed immediately
